...
These variables are not consumed by ghc-proxy itself. They are either generated by the `--claude-code` mode for terminal output, or documented in the README's `settings.json` example for users to configure Claude Code.
