...
| `--upstream-timeout` | -- | `1800` | Upstream request timeout in seconds (`0` disables). On Bun, `fetch` enforces its own ~300s ceiling, so larger values never fire there — the request is rejected at ~300s and returned as a `504`. |
